...
export class MomentoQueryInterfaceTypescript extends AbstractQueryInterface {
  ...
  /**
   * At the time of writing, Momento's `bulkDelete` only deletes a single `row` where each row is
   * a dictionary item persisted in Momento's cache. Primary key attribute and it's value must be present when
   * calling this method.
   *
   * @param tableName
   * @param identifier
